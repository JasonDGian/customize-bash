...
# permissions of the user's console profile files.
...
# ----------------------------------------------------------------------
# Installs essential packages based on the detected Linux distribution.
# It first determines the distribution type by checking for specific files:
# - For Debian-based systems (e.g., Ubuntu), it uses `apt-get` to update the package list and install `git`, `figlet`, and `nerd-fonts`.
# - For Red Hat-based systems (e.g., CentOS, Fedora), it uses `yum` to check for updates and install the same packages.
# - For Arch-based systems (e.g., Arch Linux, Manjaro), it uses `pacman` to synchronize the package database and install the packages.
# If the distribution is not recognized, it outputs an error message and advises manual installation of the required packages.
